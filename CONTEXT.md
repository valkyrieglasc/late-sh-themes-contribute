...
- Last updated: 2026-04-30 (`/public` creates opt-in rooms; Chat list hides game-backed rooms)
- Status: Active
...
- **Main responsibilities:** Multi-screen TUI over SSH (Dashboard, Chat, News, The Arcade, Rooms, Artboard), genre voting, paired browser/CLI audio control plus visualizer, real-time global chat (regular SSH chat messages support a small Markdown subset: headings, bold, italic, inline code, blockquotes, and simple `- ` list items; messages also carry simple per-user numeric reactions `1..8` rendered as footer chips beneath the message block; admins can toggle a generic per-message `pinned` flag with `Ctrl+P`, and pinned messages visible to the current user stack above the dashboard chat; `---NEWS---` cards still use their dedicated renderer; `chat_rooms.kind = 'game'` stays in the chat state for embedded Rooms chat, but is hidden from the main Chat room list/navigation and favorite-room picker), link and YouTube sharing with AI summaries/ASCII thumbnails, interactive terminal games (2048, Sudoku, Nonograms, Minesweeper, Solitaire, legacy admin-gated Arcade Blackjack), a dedicated Rooms screen for persistent game-backed rooms, plus a dedicated shared multi-user ASCII artboard screen. The Rooms list is a responsive directory: filter pills (`h`/`l` or left/right arrows cycle, `Tab`/`Shift+Tab` stay global screen switching), `/` fuzzy search by name, `n` opens an admin-only create modal with pace/stake settings (placeholder Poker/Chess/Battleship/Tron rows render as "coming soon" under a `· soon ·` divider). Wide layout (≥80 cols) shows a columned table; narrow falls back to two-line cards. Admins create, admins/mods enter; everyone else can browse but not enter. Inside an entered Blackjack room, the page splits 70/30 into a fancy table view and a chat placeholder. The table view has three render tiers picked from inner area dims: **ultra-fancy** (≥96 cols × 26 rows) gives every seat full Outline cards (9×5) side-by-side; **fancy** (≥60×22) keeps the dealer in Outline cards but renders seats with compact text cards; **compact** (below) falls back to a single-list seat layout with Info sidebar. All player info lives on the seats themselves: username/balance from `BlackjackSeat.player`, shared pre-lock stake chips from `BlackjackSeat.stake_chips`, cards, locked bet, total, stand/thinking status, and outcome chip. The bottom info bar is just `chip · phase · deal countdown`; the key-hint bar at the very bottom is phase-aware so every action has its key labeled inline. Outer Rooms block title swaps to `<room name> · <game> · Esc back` when a room is active. Up to four seats; auto-deal fires five seconds after the most recent confirmed bet. The bottom chat pane is still a placeholder. Configurable right-side panels: the global app sidebar (now playing, activity, visualizer, bonsai) plus the arcade lobby leaderboard sidebar, both default-on. Global `q` now opens a quit-confirm modal; pressing `q` again exits and `Esc` dismisses it. `@bot` mention replies now receive compact context about online non-bot members in the active room (username plus optional bio/country/timezone, capped and truncated for prompt size).
...
- `VoteService::subscribe_state()` (in `app::vote::svc`), `ArticleService::subscribe_snapshot()` (in `app::chat::news::svc`), `NotificationService::subscribe_snapshot()` (in `app::chat::notification_svc`) → shared `watch::Receiver<...Snapshot>` (durable latest state)
- `ChatService::start_user_refresh_task(user_id, room_rx)` → per-session `watch::Receiver<ChatSnapshot>` for cheap room summaries only; chat intentionally has no global `subscribe_state()` foot-gun.
- `ChatService::subscribe_usernames()` → shared `watch::Receiver<Arc<Vec<String>>>` username directory for mention autocomplete only.
...
In progress:
- **Rooms/Blackjack done:** Rooms is now the primary multiplayer table-game entry point (`Screen::Rooms`, key `4`). `RoomsService` creates persistent Blackjack tables in `game_rooms` and associated `chat_rooms(kind='game')` rows in one SQL CTE, limits each user to 3 non-closed Blackjack tables, and runs an hourly cleanup that marks tables `closed` after 12h without a `game_rooms.updated` touch. Room entry touches that timestamp. The service publishes snapshot refreshes plus `RoomsEvent` success/error. The Rooms page is a responsive directory: filter pill row at the top (`h`/`l` or left/right arrows cycle the `RoomsFilter` enum across `All`/`Blackjack`/`Poker`/`Chess`/`Battleship`/`Tron`), `/` for fuzzy substring search by display name, `n` opens the admin-only create modal, `j/k` and up/down arrows move the selection. Real Blackjack rooms render first; the placeholders from `app::rooms::mock` (Poker/Chess/Battleship/Tron with per-game `GameMeta`) render under a `· soon ·` divider, are unselectable, and only show when the active filter or `All` matches. Wide layout (≥80 cols) renders a columned table (Name · Game · Seats · Pace · Stakes · Status); narrow falls back to two-line cards. Blackjack rooms display pace/stake from `game_rooms.settings`; seats render live `occupied/max` counts from in-memory `BlackjackService` snapshots when loaded, falling back to `?/4` before a table exists in this process. Admin-only room creation, admin/mod room entry, return to the list with `Esc` (also peels back state in order: form → search → query → filter). Active room mode captures input after modals/global `Ctrl-O`, so game/room keys do not leak to app-wide hotkeys (`w`, `P`, `?`, `1-5`, `Tab`, etc.).
- **Blackjack table runtime done:** `BlackjackTableManager` lazily maps each entered Blackjack `GameRoom.id` to its own runtime `BlackjackService` and injects a shared `BlackjackPlayerDirectory` for DB-backed username/chip lookups. The directory is an SSH app wrapper only; the SQL-backed lookup lives in `late_core::models::blackjack::BlackjackPlayer`. Blackjack room creation uses a dedicated modal for table name, pace (`quick`, `standard`, `chill`), and stake (`10`, `50`, `100`, `500` chips); those settings are persisted in `game_rooms.settings` and applied when the room runtime is first created. Entering a room starts as a viewer; `s`/Enter takes the first open seat, up to four seats. Each `BlackjackSeat` snapshot includes optional `BlackjackPlayerInfo { user_id, username, balance }`, refreshed on sit, accepted bets, and hand settlement so other seats can render names and chip balances without relying on chat username refreshes. Seated players build a shared visible stake from a chip rack instead of typing numbers: `[ ]` or `A/D` selects a chip locally, Space tosses it onto that seat's `stake_chips`, Backspace pulls one chip back, `c` clears the stake, and Enter locks the service-owned stake into a pending bet. Each confirmed stake restarts the table's pace-specific betting window so other seated players can still bet concurrently until auto-deal. After the deal, all betting seats can hit/stand their own hands in parallel; the dealer resolves once every betting hand has stood, busted, or naturally settled. `l` leaves the seat when safe, and `Esc` leaves the room/view. Blackjack now supports multi-seat bets, per-seat hands, parallel player actions, dealer resolution, and per-user chip settlement.
- **Blackjack in-room view (table feel) done:** `app/rooms/blackjack/ui.rs` picks one of three render tiers from inner area dimensions: **ultra-fancy** (≥96 cols × 26 rows + 4 seats fit at 22-col panels) renders both dealer and every seat with full `AsciiCardTheme::Outline` cards (9×5) side-by-side; **fancy** (≥60×22) keeps the dealer Outline but seats render compact text cards in 12-col panels; **compact** falls back to the legacy `draw_game_frame` Info-sidebar layout. The outer Rooms block title swaps to `<room name> · <game> · Esc back` whenever an active room is set (see `rooms/ui.rs::rooms_title`). Active-room area is split 70/30 (game / chat placeholder). All player-specific info renders **on the seat**: username and chip balance from `BlackjackSeat.player` when available, shared pre-lock stake chips from `BlackjackSeat.stake_chips`, cards, total, bet, and outcome. Each seat panel has a phase-aware border color (green = you, amber = active turn, dim = open). The bottom info bar is just `chip · phase · deal countdown`; Bal/stake/locked were intentionally dropped from it. The very bottom row is a phase-aware key hint line so every actionable key is labeled inline.
- **Rooms/Blackjack not done:** Server restart still drops in-memory Blackjack table state; entering an existing room creates a fresh runtime service. Future durable games should restore from game-specific persisted state inside their manager. The active room bottom 30% is still a placeholder showing the associated `chat_room_id`; real room-scoped chat rendering is not wired yet. AFK/disconnect handling is still missing.
...
### Chat service cleanup / scoped loading (active follow-up)

The old per-session `list_chat_rooms` refresh path mixed global data, per-user room summaries, discover metadata, and per-room message tails into one heavy polling unit. That shape caused DB pool pressure and memory spikes when many PTYs refreshed together. The current contract is scoped instead:

- **Username directory:** one service-owned `watch<Arc<Vec<String>>>` refreshes all usernames every 30s for instant mention autocomplete only. It is not embedded in per-user chat snapshots.
- **Per-user summary:** joined rooms, unread counts, DM peer/current-user display data, bonsai glyphs for those users, ignored user IDs, and general room ID. This remains the normal 10s recovery poll and does not load message history.
- **Per-room tails:** loaded explicitly for the visible chat/dashboard room via `load_room_tail_task`; tails are capped at 500 messages and include author usernames, author bonsai glyphs, and reaction summaries for those messages.
- **Discover:** public-room discover metadata loads only when the synthetic Discover row is selected.
- **Events as patches:** broadcasts append/update/delete local state and carry author display/glyph metadata for new/edited messages. Tail loads merge/dedupe with already-applied live events instead of replacing them blindly.

The refresh task itself remains load-bearing for dropped broadcast recovery of room membership/unread state, but it is no longer the unit that rebuilds the whole chat application state for every user.
...
- **Blackjack in-room render tier is dimension-driven:** `app/rooms/blackjack/ui.rs` picks ultra-fancy/fancy/compact from `inner` area dimensions (`ULTRA_FANCY_MIN_WIDTH = 96`, `ULTRA_FANCY_MIN_HEIGHT = 26`, `FANCY_MIN_WIDTH = 60`, `FANCY_MIN_HEIGHT = 22`). Resizing a session crosses tiers without re-fetching anything. The seat panel widths/heights (`SEAT_PANEL_WIDTH_OUTLINE = 22 / SEAT_PANEL_HEIGHT_OUTLINE = 11` vs `SEAT_PANEL_WIDTH = 12 / SEAT_PANEL_HEIGHT = 7`) and `DEALER_BLOCK_HEIGHT = 9` are wired into the vertical `Layout::vertical` constraints; bumping any of them without bumping the min thresholds will overflow chunks and panic on `rows[N]` indexing — pick thresholds first, then panel dims.
- **Blackjack player lookup is not a render concern:** seat username/balance data is loaded through `BlackjackPlayerDirectory`, which delegates SQL to `late_core::models::blackjack::BlackjackPlayer`. The UI should read `BlackjackSeat.player` and fall back only when the seat has not been hydrated yet. Do not add per-render user/chip queries.
- **Blackjack player info lives on seats, not the bottom bar:** username, hand, total, bet, balance, outcome chip, and service-owned pre-lock `stake_chips` all render inside the seat panel. The bottom info bar is intentionally minimal: `chip · phase · deal countdown`. Adding back Bal/stake/locked there will duplicate state and confuse the eye — extend the seat row instead.
- **Blackjack stake chips are service-owned seat data:** chip selection stays client-local (`selected_chip_index`), but thrown chips live on `SeatState.stake_chips` and surface through `BlackjackSeat.stake_chips` so every subscriber sees pre-lock stakes. Keep Space/Backspace/Clear mutations in `BlackjackService`; do not reintroduce client-local stake as the authority.
- **Blackjack balances are seat player data:** every seat can carry `BlackjackPlayerInfo.balance` once hydrated, while `BlackjackSnapshot.balance` remains legacy/current-user state for compact flows. Keep balance refreshes in service events (sit, accepted bets, settlement), not in rendering.
- **Rooms list filter cycle is `h`/`l` or left/right arrows — `Tab`/`Shift+Tab` stay global:** the global screen switcher claims `Tab`/`Shift+Tab` and `1-5` (Dashboard/Chat/Games/Rooms/Artboard), so the rooms filter row avoids those keys. `RoomsFilter::cycle(forward)` wraps both directions. `Esc` peels back state in this order: form open → search active → search query → non-All filter → leave room. Don't add filter digit shortcuts without first carving them out of `handle_global_key`.
- **Rooms placeholders are UI-only:** `app::rooms::mock::PLACEHOLDERS` (Poker/Chess/Battleship/Tron) are static `&[PlaceholderKind]` interleaved into the rendered list under a `· soon ·` divider. They are *not* in `game_rooms`, do not have UUIDs, and are unselectable — `rooms_selected_index` only counts visible *real* rooms after filter+search. `visible_real_count` / `visible_real_room_at` are duplicated between `state.rs` and `input.rs` because each side computes against the same filter+search inputs; if you change one, change the other or selection clamps will desync.
...
- **Pinned chat messages are global:** `chat_messages.pinned` is a message-level flag, not tied to `#announcements` or room membership. Only admins can toggle it (`Ctrl+P` on the selected message). Dashboard rendering stacks pinned messages loaded through the separate `load_pinned_messages_task` watch pipeline, not the 10s summary snapshot or chat event broadcast.
- **Snapshot merge for empty rooms:** chat summary snapshots intentionally carry empty message vectors. `ChatState::merge_rooms` preserves cached tails from explicit `RoomTailLoaded` events so the 10s summary refresh never flash-clears visible history.
...
- **Transcript render cost is cache-sensitive:** loaded room tails are broadcast-driven and hard-capped at 500 messages per room. The chat UI caches wrapped transcript rows for the dashboard card and the active room; invalidation must track width, message content/order, usernames, badges, and bonsai glyphs. Message tails are fetched only by explicit visible-room tail loads, not by the 10s summary refresh.
...
- `list_chat_rooms` (`late-ssh/src/app/chat/svc.rs:179`) runs every 10s per session and unconditionally fetches:
  ...
  - up to 100 pinned messages visible to the user for the dashboard pinned stack
...
| **Rooms** | 4 | Active | Responsive room directory + active-table view. List has a filter pill row (`h`/`l` or left/right arrows cycle `RoomsFilter`), `/` fuzzy search, `n` admin-only create modal, `j/k` and up/down arrows nav, `Esc` peels state. Wide layout shows columned table; narrow falls back to two-line cards. `app::rooms::mock` placeholders for Poker/Chess/Battleship/Tron render under a `· soon ·` divider when filter allows. Persists `game_rooms`, admins create Blackjack tables (modal: name + pace + stake), admins/mods enter as viewers, sit in one of four seats, place per-seat bets, auto-deals five seconds after the most recent confirmed bet via `BlackjackTableManager`-bound `BlackjackService`. Active room area splits 70/30 (game / chat placeholder); outer block title swaps to `<room name> · <game> · Esc back`. Game view picks ultra-fancy / fancy / compact tier from inner dims (see invariant in §8.4). All player info on seats; bottom bar is `chip · phase · deal`. Active Rooms mode captures input; only global `Ctrl-O` reaches settings. |
